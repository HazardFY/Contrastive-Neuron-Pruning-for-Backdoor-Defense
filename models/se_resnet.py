...
class CifarSEBasicBlock(nn.Module):
    def __init__(self, inplanes, planes, stride=1, reduction=16,norm_layer=nn.BatchNorm2d):
...
class CifarSEResNet(nn.Module):
    def __init__(self, block, n_size, num_classes=10, reduction=16,norm_layer=nn.BatchNorm2d):
        ...
        self.layer1 = self._make_layer(
            block, 16, blocks=n_size, stride=1, reduction=reduction,norm_layer=self._norm_layer)
        self.layer2 = self._make_layer(
            block, 32, blocks=n_size, stride=2, reduction=reduction,norm_layer=self._norm_layer)
        self.layer3 = self._make_layer(
            block, 64, blocks=n_size, stride=2, reduction=reduction,norm_layer=self._norm_layer)
    ...
    def _make_layer(self, block, planes, blocks, stride, reduction,norm_layer):
        strides = [stride] + [1] * (blocks - 1)
        layers = []
        for stride in strides:
            layers.append(block(self.inplane, planes, stride, reduction,norm_layer))
            self.inplane = planes
