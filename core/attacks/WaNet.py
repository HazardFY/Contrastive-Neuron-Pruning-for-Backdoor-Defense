...
class AddDatasetFolderTrigger(AddTrigger):
    ...
    def __call__(self, img):
        ...
        if type(img) == PIL.Image.Image:
            ...
            img = F.convert_image_dtype(img, torch.float)# !将img/255
...
class PoisonedDatasetFolder(DatasetFolder):
    def __init__(self,
        ...
        self.poisoned_transform.transforms.insert(poisoned_transform_index, AddDatasetFolderTrigger(identity_grid, noise_grid,  noise=False,opt=opt))
        #add noise transform
        self.poisoned_transform_noise.transforms.insert(poisoned_transform_index, AddDatasetFolderTrigger(identity_grid, noise_grid,  noise=True,opt=opt))
...
def CreatePoisonedDataset(benign_dataset, y_target, poisoned_rate, identity_grid, noise_grid, noise,opt, poisoned_transform_index, poisoned_target_transform_index):
    class_name = type(benign_dataset)
    if class_name == DatasetFolder:
        return PoisonedDatasetFolder(benign_dataset, y_target, poisoned_rate, identity_grid, noise_grid, noise,opt, poisoned_transform_index, poisoned_target_transform_index)
