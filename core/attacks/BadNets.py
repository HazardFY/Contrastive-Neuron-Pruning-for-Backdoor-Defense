...
class AddCIFAR10Trigger(AddTrigger):
    ...
    def __call__(self, img):
        ...
        img = self.add_trigger(img) # 作者采用的是类似于blended的混合方法
        img = Image.fromarray(img.permute(1, 2, 0).numpy())
        return img
...
class PoisonedDatasetFolder(DatasetFolder):
    def __init__(self,
        ...
        total_num = len(self.samples) # PoisonedDatasetFolder在继承的时候，重新使用父类方法 重新进行初始化。因此对于 如果是经过处理后的父类 继承后是没有处理的状态
...
class PoisonedCIFAR10(CIFAR10):
    def __init__(self,
        ...
        super(PoisonedCIFAR10, self).__init__(
            benign_dataset.root,# 数据集所在目录的根目录 如果download设置为True。“cifar-10-batches-py '”存在，则将被保存至该目录
            benign_dataset.train,# 如果为True，则从训练集创建数据集，否则从测试集创建。
            benign_dataset.transform,# 一个接受PIL图像的函数/变换 并返回转换后的版本。 如”transforms“,“RandomCrop”
            benign_dataset.target_transform, # (可调用，可选):一个作用于目的转换函数
            download=True) # 如果为true，则从internet下载数据集 ,将其放在根目录中。 如果数据集已经下载，则不会 再次下载。
        total_num = len(benign_dataset)
        poisoned_num = int(total_num * poisoned_rate)
        assert poisoned_num >= 0, 'poisoned_num should greater than or equal to zero.'
        tmp_list = list(range(total_num))
        ...
        if self.transform is None:
            ...
            self.poisoned_transform = copy.deepcopy(self.transform)
        # 将trigger添加，作为数据增广的一步，添加dataset对应的图像中
...
def CreatePoisonedDataset(benign_dataset, y_target, poisoned_rate, pattern, weight, opt,poisoned_transform_index, poisoned_target_transform_index):
    class_name = type(benign_dataset)
    if class_name == DatasetFolder:
        return PoisonedDatasetFolder(benign_dataset, y_target, poisoned_rate, pattern, weight,opt, poisoned_transform_index, poisoned_target_transform_index)
