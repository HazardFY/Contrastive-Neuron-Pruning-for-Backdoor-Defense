...
class Base(object):
    ...
    def train(self, schedule=None):
        ...
        work_dir = osp.join(self.current_schedule['save_dir'], self.current_schedule['experiment_name'] + '_'+ self.current_schedule['dataset']+ '_'+self.current_schedule['attack_method'] +'_'+time.strftime("%Y-%m-%d_%H:%M:%S", time.localtime()))
        os.makedirs(work_dir, exist_ok=True)

        # save record
        if 'noise_grid' in self.current_schedule.keys():
            torch.save(self.current_schedule['identity_grid'],osp.join(work_dir,'identity_grid.pth'))
            torch.save(self.current_schedule['noise_grid'],osp.join(work_dir,'noise_grid.pth'))
            self.current_schedule.pop('identity_grid')
            self.current_schedule.pop('noise_grid')
        with open(os.path.join(work_dir, "opt.json"), "w+") as f:
        ...
        for i in range(self.current_schedule['epochs']):
            ...
            if (i + 1) % self.current_schedule['save_epoch_interval'] == 0:
                ...
                ckpt_model_filename = "ckpt_epoch_" + str(i+1) +"_BA"+ str(BA)+"_ASR"+str(ASR)+".pth"
